...
{currentUser?.role === "author" && (
  <div className="d-flex align-items-center gap-2">
    {/* Edit Button */}
    <button className="btn btn-light" onClick={enableEdit} title="Edit Article">
      <FaEdit className="text-warning fs-4" />
    </button>

    {/* Conditional Delete/Restore Button */}
    {state?.isArticleActive ? (
      <button className="btn btn-light" onClick={deleteArticle} title="Delete Article">
        <MdDelete className="text-danger fs-4" />
      </button>
    ) : (
      <button className="btn btn-light" onClick={restoreArticle} title="Restore Article">
        <MdRestore className="text-info fs-4" />
          ...
          <div>
            <div className="comments my-4">
              {
                state.comments.length === 0 ? <p className='display-4'>No comments yet..</p> :
